...
# class Eval():
#     def __init__(self, model, accuracies, f1_scores, iter, batch_size, sigma, epoch_timer_list):
#         self.epoch_timer_list = epoch_timer_list
...
#         self.sigma = sigma
#         self.model = model
#         self.hidden_size = model.config.hidden_size  # This gets the correct hidden size for any model
#         self.classifier = nn.Linear(self.hidden_size, num_labels).to(gpu)
...
#     def __call__(self, epoch=-1, steps=-1):
#         current_time = time.time()
#         epoch_time = current_time - self.last_epoch_time
#         self.epoch_timer_list.append(epoch_time)
#         self.last_epoch_time = current_time
#         print(f"Epoch {trainer.state.epoch} took {epoch_time:.2f} seconds.")

#         train_emb = self._encode(train_input_ids, train_attention_mask, batch_size=128).to(gpu)
#         val_emb = self._encode(val_input_ids, val_attention_mask, batch_size=128).to(gpu)
#         test_emb = self._encode(test_input_ids, test_attention_mask, batch_size=128).to(gpu)

#         prev_loss = float('inf')  # Initialize previous loss as infinity
#         threshold = 0.001  # Minimum required improvement to continue training
...
#             logits = self.classifier(train_emb)
#             train_loss = F.cross_entropy(logits, train_labels)
#             # Check early stopping condition
#             if abs(prev_loss - train_loss.item()) < threshold:
#                 print(f"Stopping early at iteration {i} as loss improvement is less than {threshold}.")
#                 break
#             train_loss.backward()
#             self.classifier_optimizer.step()
#             prev_loss = train_loss.item()
#             print(f"Iteration {i}: Train Loss = {train_loss.item():.4f}")
...
#             # Validation
#             optimizer.zero_grad()
#             val_logits = self.classifier(val_emb)
#             val_logits = val_logits * torch.tensor(class_weights, dtype=torch.float32).to(gpu)
#             val_labels = labels['val'].to(gpu)
#             val_loss = F.cross_entropy(val_logits, val_labels, weight=torch.tensor(class_weights, dtype=torch.float32).to(gpu))
#             val_ypred = torch.argmax(val_logits, dim=1).cpu()
#             val_acc = accuracy_score(val['label'], val_ypred)
#             val_f1 = f1_score(val['label'], val_ypred, average='macro')

#             # Test
#             test_logits = self.classifier(test_emb)
#             val_logits = val_logits * torch.tensor(class_weights, dtype=torch.float32).to(gpu)
#             test_labels = labels['test'].to(gpu)
#             test_loss = F.cross_entropy(test_logits, test_labels,weight=torch.tensor(class_weights, dtype=torch.float32).to(gpu))
#             test_ypred = torch.argmax(test_logits, dim=1).cpu()
#             test_acc = accuracy_score(test['label'], test_ypred)
#             test_f1 = f1_score(test['label'], test_ypred, average='macro')

#         self.f1_scores.append(val_f1)
#         self.accuracies.append(val_acc)

#         logger.info(
#             f"\rEpoch:{trainer.state.epoch},"
#             f"Val Accuracy:{val_acc:.4f}, val F1-score:{val_f1:.4f}, val loss:{val_loss:.4f}, "
#             f"Test accuracy:{test_acc:.4f}, test f1-score:{test_f1:.4f}, test loss:{test_loss:.4f}")
